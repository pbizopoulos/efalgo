...
tmp/check-done: .gitignore README tmp
	...
	test -s .deploy-requirements.sh && nix run nixpkgs#shellcheck .deploy-requirements.sh && nix run nixpkgs#shfmt -- --posix --write .deploy-requirements.sh || true
	test -s .deploy.sh && nix run nixpkgs#shellcheck .deploy.sh && nix run nixpkgs#shfmt -- --posix --write .deploy.sh || true
